...
export const config  = {
    ...
    API_CONFIG: {
        ...
        staticBaseUrl: 'https://example.com/api/v1',
...
// 仅保留发布页自定义卡片，隐藏历史默认的 GitHub 与 Telegram 卡片。
if (config.MORE_PAGE_CONFIG?.customCards) {
    config.MORE_PAGE_CONFIG.customCards = config.MORE_PAGE_CONFIG.customCards.filter(
        card => card.id !== 'github' && card.id !== 'telegram'
    );
}
